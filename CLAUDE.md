...
- A driving-task CarPlay app (entitlement `com.apple.developer.carplay-driving-task`; the scene is declared in `KyoNoAshiato/Info.plist`). The root is a `CPInformationTemplate` (state / start time / distance / elapsed, plus 出発・一時停止/再開・到着・地図 buttons) refreshed by a 10-second timer while recording and by `.locationManagerRecordingDidChange`. Driving-task guidelines cap periodic refreshes, so data items are re-applied at most once every 10 seconds and POIs at most once every 60 seconds (recording-state changes bypass the gate since they are user-initiated; durations are shown in minutes because seconds can't be kept current). The 地図 button pushes a `CPPointOfInterestTemplate` showing the start pin, up to 10 distance-thinned 👣 waypoint pins, and a current-location dot (`CPPointOfInterestTemplate` caps POIs at 12). Starting from CarPlay is guarded by `locationManager.canRecordLocation`.
...
- **CarPlay** templates (`CPInformationTemplate`, `CPPointOfInterestTemplate`) for the in-car screen
