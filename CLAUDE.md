...
- A driving-task CarPlay app (entitlement `com.apple.developer.carplay-driving-task`; the scene is declared in `KyoNoAshiato/Info.plist`). The root is a `CPInformationTemplate` (state / start time / distance / elapsed, plus 出発・一時停止/再開・到着・周辺 buttons — the bottom row caps at 3, and 周辺 is present in every state so the layout barely changes between idle and recording) refreshed by a 10-second timer while recording and by `.locationManagerRecordingDidChange`. Driving-task guidelines cap periodic refreshes, so data items are re-applied at most once every 10 seconds and POIs at most once every 60 seconds (recording-state changes bypass the gate since they are user-initiated, so durations show seconds but tick in 10-second jumps). Starting from CarPlay is guarded by `locationManager.canRecordLocation`.
- The CarPlay map screen only ever shows nearby-place search results. Rendering the recorded route there (start pin, waypoints, current location) was removed on purpose: the POI template is meant for places you pick to act on, a breadcrumb trail is a review feature rather than a driving task, and dropping it makes the idle and recording UI identical.
- **Nearby search** — the 周辺 button opens the map screen on the category you looked at last (`placeCategory`), and the map screen's nav bar lists the other two categories so any of 駐車場 / 給油 / EV充電 is one tap away. An action sheet chooser was tried and dropped: on an 800x480 screen the title plus four buttons don't fit and the cancel action goes missing. `PlaceSearchService.swift` wraps `MKLocalPointsOfInterestRequest` per category (one instance each, so caches are independent): 9 spots at most within 1.5 km, falling back to 3 km when nothing is nearby, picked by distance from the searched center but labelled with the distance from the driver. Pins carry the distance-order number (matching the card title) in a category color — parking indigo, fuel teal, EV green — and the highlighted one is drawn orange. Panning the map re-searches the new area (1.5 s debounce, and never more often than the 60-second POI cap); nothing else replaces the POIs, so location updates no longer touch the map. The map camera can't be controlled at all: `selectedIndex` only highlights, re-applying POIs doesn't reframe, and popping/pushing the template to force a reframe breaks navigation — see the CarPlay POI template limits in memory.
